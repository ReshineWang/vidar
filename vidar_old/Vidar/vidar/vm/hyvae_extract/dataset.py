...
class VideoDataset(Dataset):
    def __init__(
        ...
        if enable_multi_aspect_ratio:
            ...
            # print(f'all the bucket:',self.buckets)
        # image preprocess
    ...
    def preprocess_url(self, data_json_path):
        ...
        # keep the last file name and the former path name 
        folder_name = video_path.split("/")[-2]
        file_name = video_path.split('/')[-1].split('.')[0]
        video_id = folder_name + '_' + file_name
    ...
    def get_item(self, idx):
        ...
        if len(video_reader) < 120:
            print(f'Error: video: {video_item["video_path"]} with frame {len(video_reader)}, require video clip with more than 120 frames')
            return None, video_item["videoid"], video_item["video_path"], video_item["prompt"], False
        
        # uniformly sample 60 frames 
        stride: float = (len(video_reader) - 1) / (self.sample_n_frames - 1) 
        batch_index = [round(i * stride) for i in range(self.sample_n_frames)]
