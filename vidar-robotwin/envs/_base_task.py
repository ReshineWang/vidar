...
from script.eval_policy import clean_print
...
class Base_Task(gym.Env):
    ...
    def set_instruction(self, instruction=None):
        ...
        for i, line in enumerate(lines):
            if line.startswith("def play_once"):
                function_start_idx = i + 1
            elif line.startswith("def ") and function_start_idx is not None:
                function_end_idx = i
                break
        lines = lines[function_start_idx: function_end_idx]
        target_idx = None
        for i, line in enumerate(lines):
            if lines[i].startswith("for"):
                break
            elif lines[i].startswith("self.arm_tag = ArmTag"):
                target_idx = i
                break
            elif lines[i].startswith("arm_tag = ArmTag"):
        ...
        if target_idx is not None:
            try:
                ...
                print(f"Error occurred while executing line: {lines[i]} for task {self.task_name}")
                print(e)

        if self.task_name in ["place_a2b_left", "place_a2b_right", "put_object_cabinet"]:
            self.full_instruction = self.instruction.lower()
        else:
            with open(os.path.join(os.path.dirname(base_dir), f"description/task_instruction/{self.task_name}.json"), "r") as f:
                task_info = json.load(f)
            self.full_instruction = task_info["full_description"].replace("<", "").replace(">", "")
            self.full_instruction = self.full_instruction[0].lower() + self.full_instruction[1:]
        if hasattr(self, "arm_tag"):
            if str(self.arm_tag) == "left":
                self.full_instruction = f"using left arm, {self.full_instruction}"
            elif str(self.arm_tag) == "right":
                self.full_instruction = f"using right arm, {self.full_instruction}"
        if not self.full_instruction.endswith("."):
            self.full_instruction = self.full_instruction + "."
    ...
    def take_action(self, action, action_type:Literal['qpos', 'ee', 'delta_ee']='qpos'):  # action_type: qpos or ee
        ...
        clean_print(f"step: \033[92m{self.take_action_cnt} / {self.step_lim}\033[0m", end="\r")
